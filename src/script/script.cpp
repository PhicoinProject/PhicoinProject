...
const char* GetOpName(opcodetype opcode)
{
    ...
    /** PHI START */
    case OP_PHI_ASSET              : return "OP_PHI_ASSET";
    /** PHI END */
    ...
    if (this->size() > 31) {
        if ((*this)[25] == OP_PHI_ASSET) { // OP_PHI_ASSET is always in the 25 index of the script if it exists
            ...
            if (index > 0) {
                ...
                if ((*this)[index] == PHI_T) { // Transfer first anticipating more transfers than other assets operations
                    nType = TX_TRANSFER_ASSET;
                    return true;
                } else if ((*this)[index] == PHI_Q && this->size() > 39) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = false;
                    return true;
                } else if ((*this)[index] == PHI_O) {
                    nType = TX_NEW_ASSET;
                    fIsOwner = true;
                    return true;
                } else if ((*this)[index] == PHI_R) {
...
{
    // 1 OP_PHI_ASSET followed by two OP_RESERVED + atleast 4 characters for the restricted name $ABC
    return (this->size() > 6 &&
            (*this)[0] == OP_PHI_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] == OP_RESERVED);
...
{
    // 1 OP_PHI_ASSET followed by one OP_RESERVED
    return (this->size() > 3 &&
            (*this)[0] == OP_PHI_ASSET &&
            (*this)[1] == OP_RESERVED &&
            (*this)[2] != OP_RESERVED);
...
{
    CAmount nAmount;
    return (size() > 0 && *begin() == OP_RETURN) || (size() > 0 && *begin() == OP_PHI_ASSET) || (size() > MAX_SCRIPT_SIZE) || (GetAssetAmountFromScript(*this, nAmount) && nAmount == 0);
}
