...
uint256 PHIHASHHash(const CBlockHeader& blockHeader, uint256& mix_hash)
{
    ...
    uint256 nHeaderHash = blockHeader.GetPHIHASHHeaderHash();
    const auto header_hash = to_hash256(nHeaderHash.GetHex());

    // PhiHash hash
    const auto result = phihash::hash(*context, blockHeader.nHeight, header_hash, blockHeader.nNonce64);
...
uint256 PHIHASHHash_OnlyMix(const CBlockHeader& blockHeader)
{
    // Build the header_hash
    uint256 nHeaderHash = blockHeader.GetPHIHASHHeaderHash();
    const auto header_hash = to_hash256(nHeaderHash.GetHex());
    ...
    // PhiHash hash
    const auto result = phihash::hash_no_verify(blockHeader.nHeight, header_hash, to_hash256(blockHeader.mix_hash.GetHex()), blockHeader.nNonce64);
