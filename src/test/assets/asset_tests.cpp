...
BOOST_FIXTURE_TEST_SUITE(asset_tests, BasicTestingSetup)
    ...
    BOOST_AUTO_TEST_CASE(name_validation_tests)
    {
        ...
        //- Versions of PHICOIN NOT allowed
        BOOST_CHECK(!IsAssetNameValid("PHI", type));
        BOOST_CHECK(!IsAssetNameValid("PHICOIN", type));
        BOOST_CHECK(!IsAssetNameValid("PHICOIN", type));

        //- Versions of PHICOIN ALLOWED
        BOOST_CHECK(IsAssetNameValid("PHICOIN.COIN", type));
        BOOST_CHECK(IsAssetNameValid("PHICOIN_COIN", type));
        BOOST_CHECK(IsAssetNameValid("PHISPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDERPHI", type));
        BOOST_CHECK(IsAssetNameValid("PHICOINSPYDER", type));
        BOOST_CHECK(IsAssetNameValid("SPYDEPHICOIN", type));
        BOOST_CHECK(IsAssetNameValid("BLACK_PHICOINS", type));
        BOOST_CHECK(IsAssetNameValid("SEPHIOT", type));
    ...
    BOOST_AUTO_TEST_CASE(transfer_asset_coin_test)
    {
        ...
        CAssetTransfer asset("PHICOIN", 1000);
    ...
    BOOST_AUTO_TEST_CASE(new_asset_coin_test)
    {
        ...
        // Create the asset scriptPubKey
        CNewAsset asset("PHICOIN", 1000, 8, 1, 0, "");
        CScript scriptPubKey = GetScriptForDestination(DecodeDestination(GetParams().GlobalBurnAddress()));
        asset.ConstructTransaction(scriptPubKey);
