...
#include "crypto/ethash/phihash_test_vectors.hpp"

#include <array>

BOOST_FIXTURE_TEST_SUITE(phihash_tests, BasicTestingSetup)

BOOST_AUTO_TEST_CASE(phihash_l1_cache)
{
...
BOOST_AUTO_TEST_CASE(phihash_hash_empty)
{
    auto& context = get_ethash_epoch_context_0();
    ...
    while (count > 0) {
        result = phihash::hash(context, count, {}, 0);
        --count;
    }
...
BOOST_AUTO_TEST_CASE(phihash_hash_30000)
{
    ...
    const auto result = phihash::hash(*context, block_number, header, nonce);
...
BOOST_AUTO_TEST_CASE(phihash_hash_and_verify)
{
    ethash::epoch_context_ptr context{nullptr, nullptr};

    for (auto& t : phihash_hash_test_cases)
    {
        ...
        auto final_failure = phihash::verify(
...
BOOST_AUTO_TEST_CASE(phihash_search)
{
    ...
    auto sr = phihash::search(ctx, 0, {}, boundary, 700, 100);
    auto srl = phihash::search_light(ctxl, 0, {}, boundary, 700, 100);
    ...
    sr = phihash::search(ctx, 0, {}, boundary, 300, 100);
    srl = phihash::search_light(ctxl, 0, {}, boundary, 300, 100);
