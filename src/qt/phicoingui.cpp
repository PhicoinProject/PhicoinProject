...
const std::string PHICOINGUI::DEFAULT_UIPLATFORM =
#if defined(Q_OS_MAC)
        "macosx"
...
const QString PHICOINGUI::DEFAULT_WALLET = "~Default";
...
std::array<CurrencyUnitDetails, 5> CurrencyUnits::CurrencyOptions = { {
    { "BTC",    "PHIBTC"  , 1,          8},
    { "mBTC",   "PHIBTC"  , 1000,       5},
    { "µBTC",   "PHIBTC"  , 1000000,    2},
    { "Satoshi","PHIBTC"  , 100000000,  0},
    { "USDT",   "PHIUSDT" , 1,          5}
} };

static bool ThreadSafeMessageBox(PHICOINGUI *gui, const std::string& message, const std::string& caption, unsigned int style);

PHICOINGUI::PHICOINGUI(const PlatformStyle *_platformStyle, const NetworkStyle *networkStyle, QWidget *parent) :
    QMainWindow(parent),
    enableWallet(false),
...
void PHICOINGUI::createActions()
{
    ...
    sendCoinsAction->setStatusTip(tr("Send coins to a PHICOIN address"));
    ...
    receiveCoinsAction->setStatusTip(tr("Request payments (generates QR codes and phicoin: URIs)"));
    ...
    transferAssetAction->setStatusTip(tr("Transfer assets to PHI addresses"));
...
//    messagingAction->setShortcut(QKeySequence(Qt::ALT + Qt::Key_9));
    ...
    signMessageAction->setStatusTip(tr("Sign messages with your PHICOIN addresses to prove you own them"));
    verifyMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/verify"), tr("&Verify message..."), this);
    verifyMessageAction->setStatusTip(tr("Verify messages to ensure they were signed with specified PHICOIN addresses"));
    ...
    openAction->setStatusTip(tr("Open a phicoin: URI or payment request"));

    showHelpMessageAction = new QAction(platformStyle->TextColorIcon(":/icons/info"), tr("&Command-line options"), this);
    showHelpMessageAction->setMenuRole(QAction::NoRole);
    showHelpMessageAction->setStatusTip(tr("Show the %1 help message to get a list with possible PHICOIN command-line options").arg(tr(PACKAGE_NAME)));
...
void PHICOINGUI::createToolBars()
{
    if(walletFrame)
    {
        QSettings settings;
        bool IconsOnly = settings.value("fToolbarIconsOnly", true).toBool();
        ...
        if(IconsOnly) {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/phitext")));
        }
        else {
            labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/phicointext")));
        }
...
//        m_toolbar->addAction(votingAction);
        ...
        QVBoxLayout* phicoinLabelLayout = new QVBoxLayout(toolbarWidget);
        phicoinLabelLayout->addWidget(labelToolbar);
        phicoinLabelLayout->addWidget(m_toolbar);
        ...
        // labelCurrentMarket->setText(tr("PHICOIN Market Price"));
        ...
        comboPHIUnit = new QComboBox(headerWidget);
        QStringList list;
        for(int unitNum = 0; unitNum < CurrencyUnits::count(); unitNum++) {
            list.append(QString(CurrencyUnits::CurrencyOptions[unitNum].Header));
        }
        comboPHIUnit->addItems(list);
        comboPHIUnit->setFixedHeight(26);
        comboPHIUnit->setContentsMargins(5,0,0,0);
        comboPHIUnit->setStyleSheet(STRING_LABEL_COLOR);
        comboPHIUnit->setFont(currentMarketFont);

        labelVersionUpdate->setText("<a href=\"https://github.com/PhicoinProject/phicoin/releases\">New Wallet Version Available</a>");
        ...
        // priceLayout->addWidget(comboPHIUnit, 0 , Qt::AlignBottom| Qt::AlignLeft);
        ...
        connect(comboPHIUnit, SIGNAL(activated(int)), this, SLOT(currencySelectionChanged(int)));
        // Create the timer
        ...
        // Get the latest PHICOIN release and let the user know if they are using the latest version
        // Network request code for the header widget
                       ...
                       if (key == "tag_name") {
                           ...
                           if (fNewSoftwareFound) {
                               ...
                               if (GetRandInt(2) == 1) {
                                   ...
                                   if (fRet) {
                                       QString link = "https://github.com/PhicoinProject/phicoin/releases";
...
void PHICOINGUI::updateIconsOnlyToolbar(bool IconsOnly)
{
    if(IconsOnly) {
        labelToolbar->setPixmap(QPixmap::fromImage(QImage(":/icons/phitext")));
        m_toolbar->setMaximumWidth(65);
...
void PHICOINGUI::setClientModel(ClientModel *_clientModel)
{
...
#ifdef ENABLE_WALLET
bool PHICOINGUI::addWallet(const QString& name, WalletModel *walletModel)
{
    if(!walletFrame)
        return false;
...
bool PHICOINGUI::setCurrentWallet(const QString& name)
{
    if(!walletFrame)
        return false;
...
void PHICOINGUI::removeAllWallets()
{
    if(!walletFrame)
        return;
    setWalletActionsEnabled(false);
...
void PHICOINGUI::setWalletActionsEnabled(bool enabled)
{
...
void PHICOINGUI::createTrayIcon(const NetworkStyle *networkStyle)
{
#ifndef Q_OS_MAC
...
void PHICOINGUI::createTrayIconMenu()
{
#ifndef Q_OS_MAC
...
#ifndef Q_OS_MAC
void PHICOINGUI::trayIconActivated(QSystemTrayIcon::ActivationReason reason)
{
...
void PHICOINGUI::optionsClicked()
{
...
void PHICOINGUI::showDebugWindow()
{
...
void PHICOINGUI::showDebugWindowActivateConsole()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_CONSOLE);
    showDebugWindow();
}

void PHICOINGUI::showWalletRepair()
{
    rpcConsole->setTabFocus(RPCConsole::TAB_REPAIR);
    showDebugWindow();
}

void PHICOINGUI::showHelpMessageClicked()
{
    helpMessageDialog->show();
}

#ifdef ENABLE_WALLET
void PHICOINGUI::openClicked()
{
...
void PHICOINGUI::gotoOverviewPage()
{
    overviewAction->setChecked(true);
    if (walletFrame) walletFrame->gotoOverviewPage();
}

void PHICOINGUI::gotoHistoryPage()
{
    historyAction->setChecked(true);
    if (walletFrame) walletFrame->gotoHistoryPage();
}

void PHICOINGUI::gotoReceiveCoinsPage()
{
    receiveCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoReceiveCoinsPage();
}

void PHICOINGUI::gotoSendCoinsPage(QString addr)
{
    sendCoinsAction->setChecked(true);
    if (walletFrame) walletFrame->gotoSendCoinsPage(addr);
}

void PHICOINGUI::gotoSignMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoSignMessageTab(addr);
}

void PHICOINGUI::gotoVerifyMessageTab(QString addr)
{
    if (walletFrame) walletFrame->gotoVerifyMessageTab(addr);
}

/** PHI START */
void PHICOINGUI::gotoAssetsPage()
{
    transferAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoAssetsPage();
};

void PHICOINGUI::gotoCreateAssetsPage()
{
    createAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoCreateAssetsPage();
};

void PHICOINGUI::gotoManageAssetsPage()
{
    manageAssetAction->setChecked(true);
    if (walletFrame) walletFrame->gotoManageAssetsPage();
};

void PHICOINGUI::gotoRestrictedAssetsPage()
{
...
void PHICOINGUI::updateNetworkState()
{
    ...
    if (clientModel->getNetworkActive()) {
        tooltip = tr("%n active connection(s) to PHICOIN network", "", count) + QString(".<br>") + tr("Click to disable network activity.");
    } else {
...
void PHICOINGUI::setNumConnections(int count)
{
    updateNetworkState();
}

void PHICOINGUI::setNetworkActive(bool networkActive)
{
    updateNetworkState();
}

void PHICOINGUI::updateHeadersSyncProgressLabel()
{
...
void PHICOINGUI::setNumBlocks(int count, const QDateTime& blockDate, double nVerificationProgress, bool header)
{
    if (modalOverlay)
    {
        if (header)
...
void PHICOINGUI::message(const QString &title, const QString &message, unsigned int style, bool *ret)
{
    QString strTitle = tr("PHICOIN"); // default title
...
void PHICOINGUI::changeEvent(QEvent *e)
{
...
void PHICOINGUI::closeEvent(QCloseEvent *event)
{
#ifndef Q_OS_MAC // Ignored on Mac
    if(clientModel && clientModel->getOptionsModel())
    {
...
void PHICOINGUI::showEvent(QShowEvent *event)
{
...
#ifdef ENABLE_WALLET
void PHICOINGUI::incomingTransaction(const QString& date, int unit, const CAmount& amount, const QString& type, const QString& address, const QString& label, const QString& assetName)
{
    // On new transaction, make an info balloon
    QString msg = tr("Date: %1\n").arg(date);
    if (assetName == "PHI")
        msg += tr("Amount: %1\n").arg(PHICOINUnits::formatWithUnit(unit, amount, true));
    else
        msg += tr("Amount: %1\n").arg(PHICOINUnits::formatWithCustomName(assetName, amount, MAX_ASSET_UNITS, true));
...
void PHICOINGUI::dragEnterEvent(QDragEnterEvent *event)
{
    // Accept only URIs
...
void PHICOINGUI::dropEvent(QDropEvent *event)
{
    if(event->mimeData()->hasUrls())
    {
...
bool PHICOINGUI::eventFilter(QObject *object, QEvent *event)
{
...
#ifdef ENABLE_WALLET
bool PHICOINGUI::handlePaymentRequest(const SendCoinsRecipient& recipient)
{
    // URI has to be valid
...
void PHICOINGUI::setHDStatus(int hdEnabled)
{
    QString icon = "";
...
void PHICOINGUI::setEncryptionStatus(int status)
{
    switch(status)
    {
...
void PHICOINGUI::showNormalIfMinimized(bool fToggleHidden)
{
    if(!clientModel)
        return;
...
void PHICOINGUI::toggleHidden()
{
    showNormalIfMinimized(true);
}

void PHICOINGUI::detectShutdown()
{
...
void PHICOINGUI::showProgress(const QString &title, int nProgress)
{
    if (nProgress == 0)
    {
...
void PHICOINGUI::setTrayIconVisible(bool fHideTrayIcon)
{
    if (trayIcon)
    {
...
void PHICOINGUI::showModalOverlay()
{
...
static bool ThreadSafeMnemonic(PHICOINGUI *gui, unsigned int style)
{
    bool modal = (style & CClientUIInterface::MODAL);
    // The SECURE flag has no effect in the Qt GUI.
    // bool secure = (style & CClientUIInterface::SECURE);
...
void PHICOINGUI::subscribeToCoreSignals()
{
...
void PHICOINGUI::unsubscribeFromCoreSignals()
{
...
void PHICOINGUI::toggleNetworkActive()
{
    if (clientModel) {
...
void PHICOINGUI::handleRestart(QStringList args)
{
    if (!ShutdownRequested())
    ...
    menu(0)
{
    ...
    QList<PHICOINUnits::Unit> units = PHICOINUnits::availableUnits();
    int max_width = 0;
    const QFontMetrics fm(font());
    for (const PHICOINUnits::Unit unit : units)
    {
    #ifndef QTversionPreFiveEleven
        max_width = qMax(max_width, fm.horizontalAdvance(PHICOINUnits::name(unit)));
    #else
        max_width = qMax(max_width, fm.width(PHICOINUnits::name(unit)));
    #endif
    }
...
{
    menu = new QMenu(this);
    for (PHICOINUnits::Unit u : PHICOINUnits::availableUnits())
    {
        QAction *menuAction = new QAction(QString(PHICOINUnits::name(u)), this);
...
{
    setText(PHICOINUnits::name(newUnits));
}
...
void PHICOINGUI::currencySelectionChanged(int unitIndex)
{
    if(clientModel && clientModel->getOptionsModel())
    {
...
void PHICOINGUI::onCurrencyChange(int newIndex)
{
    qDebug() << "PHICOINGUI::onPriceUnitChange: " + QString::number(newIndex);
    ...
    this->comboPHIUnit->setCurrentIndex(newIndex);
    this->getPriceInfo();
}

void PHICOINGUI::getPriceInfo()
{
...
void PHICOINGUI::getLatestVersion()
{
    versionRequest->setUrl(QUrl("https://api.github.com/repos/PHICOINProject/PHICOIN/releases"));
