...
//{
//    columns[Amount] = PHICOINUnits::getAmountColumnTitle(walletModel->getOptionsModel()->getDisplayUnit());
...
QString TransactionTableModel::formatTxAmount(const TransactionRecord *wtx, bool showUnconfirmed, PHICOINUnits::SeparatorStyle separators) const
{
    QString str;
    switch(wtx->type) {
        case TransactionRecord::Issue:
        ...
        default:
            {
            str = PHICOINUnits::format(walletModel->getOptionsModel()->getDisplayUnit(), wtx->credit + wtx->debit,
                                             false, separators);
            } break;
    }
    if (showUnconfirmed) {
...
QVariant TransactionTableModel::data(const QModelIndex &index, int role) const
{
    ...
    case Qt::DisplayRole:
        ...
        case Amount:
            return formatTxAmount(rec, true, PHICOINUnits::separatorAlways);
        case AssetName:
            if (rec->assetName != "PHI")
               return QString::fromStdString(rec->assetName);
            else
               return QString(PHICOINUnits::name(walletModel->getOptionsModel()->getDisplayUnit()));
        } // no default case, so the compiler can warn about missing cases
        assert(false);
    case Qt::EditRole:
    ...
    case TxPlainTextRole:
        {
            ...
            details.append(formatTxAmount(rec, false, PHICOINUnits::separatorNever));
            return details;
        }
    case ConfirmedRole:
    ...
    case FormattedAmountRole:
        ...
        return formatTxAmount(rec, false, PHICOINUnits::separatorNever);
    case AssetNameRole:
        {
            QString assetName;
            if (rec->assetName != "PHI")
               ...
               assetName.append(QString(PHICOINUnits::name(walletModel->getOptionsModel()->getDisplayUnit())));
            return assetName;
        }
    case StatusRole:
...
QVariant TransactionTableModel::headerData(int section, Qt::Orientation orientation, int role) const
{
    if(orientation == Qt::Horizontal)
    {
        ...
        } else if (role == Qt::ToolTipRole)
        {
            ...
            case AssetName:
                return tr("The asset (or PHI) removed or added to balance.");
            }
        }
    }
    return QVariant();
