...
#include "phicoinaddressvalidator.h"
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a PHICOIN address (e.g. %1)").arg(
        QString::fromStdString(DummyAddress(GetParams()))));
#endif
    widget->setValidator(new PHICOINAddressEntryValidator(parent));
    widget->setCheckValidator(new PHICOINAddressCheckValidator(parent));
}
...
bool parsePHICOINURI(const QUrl &uri, SendCoinsRecipient *out)
{
    // return if URI is not valid or is no phicoin: URI
    if(!uri.isValid() || uri.scheme() != QString("phicoin"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!PHICOINUnits::parse(PHICOINUnits::PHI, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
bool parsePHICOINURI(QString uri, SendCoinsRecipient *out)
{
    // Convert phicoin:// to phicoin:
    //
    //    Cannot handle this later, because phicoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("phicoin://", Qt::CaseInsensitive))
    {
        uri.replace(0, 10, "phicoin:");
    }
    QUrl uriInstance(uri);
    return parsePHICOINURI(uriInstance, out);
}

QString formatPHICOINURI(const SendCoinsRecipient &info)
{
    QString ret = QString("phicoin:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(PHICOINUnits::format(PHICOINUnits::PHI, info.amount, false, PHICOINUnits::separatorNever));
        paramCount++;
    }
...
bool openPHICOINConf()
{
    boost::filesystem::path pathConfig = GetConfigFile(PHICOIN_CONF_FILENAME);
    ...
    /* Open phicoin.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "PHICOIN.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "PHICOIN (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("PHICOIN (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for PHICOIN*.lnk
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "phicoin.desktop";
    return GetAutostartDir() / strprintf("phicoin-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a phicoin.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=PHICOIN\n";
        else
            optionFile << strprintf("Name=PHICOIN (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    ...
    // loop through the list of startup items and try to find the phicoin app
...
bool GetStartOnSystemStartup()
{
    CFURLRef phicoinAppUrl = CFBundleCopyBundleURL(CFBundleGetMainBundle());
    if (phicoinAppUrl == nullptr) {
        return false;
    }
    
    LSSharedFileListRef loginItems = LSSharedFileListCreate(nullptr, kLSSharedFileListSessionLoginItems, nullptr);
    LSSharedFileListItemRef foundItem = findStartupItemInList(loginItems, phicoinAppUrl);

    CFRelease(phicoinAppUrl);
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add phicoin app to startup item list
        LSSharedFileListInsertItemURL(loginItems, kLSSharedFileListItemBeforeFirst, nullptr, nullptr, phicoinAppUrl, nullptr, nullptr);
    }
