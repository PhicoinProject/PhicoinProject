 ...
 * between phicoind, and phicoin-qt and non-server tools.
*/
...
void PrepareShutdown()
{
    ...
    FlushWallets();
#endif
    GeneratePHICOINs(false, 0, GetParams());
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), PHICOIN_PID_FILENAME));
#endif
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/PhicoinProject/phicoin>";
    const std::string URL_WEBSITE = "<https://phicoin.net>";
...
/** Sanity checks
 *  Ensure that PHICOIN is running in a usable environment with all
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    LogPrintf("Using config file %s\n", GetConfigFile(gArgs.GetArg("-conf", PHICOIN_CONF_FILENAME)).string());
    ...
    GeneratePHICOINs(fGenerate, gArgs.GetArg("-genproclimit", DEFAULT_GENERATE_THREADS), chainparams);
