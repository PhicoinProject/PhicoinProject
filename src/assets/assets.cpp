...
// DNS RFC compliant patterns for domain-like assets - STRICT MODE
// ROOT assets (gTLD): 1-64 characters, must start with letter, end with letter/digit, support hyphen
static const std::regex ROOT_NAME_CHARACTERS("^([A-Z]|[A-Z][A-Z0-9-]{0,62}[A-Z0-9])$");
// SUB assets (subdomain): 1-63 characters, can start with letter/digit, end with letter/digit, support hyphen  
static const std::regex SUB_NAME_CHARACTERS("^([A-Z0-9]|[A-Z0-9][A-Z0-9-]{0,61}[A-Z0-9])$");

static const std::regex UNIQUE_TAG_CHARACTERS("^[-A-Za-z0-9@$%&*()[\\]{}?:]+$");
static const std::regex MSG_CHANNEL_TAG_CHARACTERS("^[A-Za-z0-9]+$");
static const std::regex VOTE_TAG_CHARACTERS("^[A-Z0-9]+$");

// Restricted assets - no underscore or dot allowed
static const std::regex QUALIFIER_NAME_CHARACTERS("#[A-Z0-9-]{3,}$");
static const std::regex SUB_QUALIFIER_NAME_CHARACTERS("#[A-Z0-9-]+$");
static const std::regex RESTRICTED_NAME_CHARACTERS("\\$[A-Z0-9-]{3,}$");

// Remove patterns for double punctuation, leading/trailing punctuation since we no longer use . and _
static const std::regex DOUBLE_HYPHEN("^.*[-]{2,}.*$");
static const std::regex LEADING_HYPHEN("^[-].*$");
static const std::regex TRAILING_HYPHEN("^.*[-]$");
static const std::regex QUALIFIER_LEADING_HYPHEN("^[#\\$][-].*$"); // Used for qualifier assets, and restricted asset only
...
// Helper function to check if a name is a valid uppercase Punycode
bool IsValidUppercasePunycode(const std::string& name)
{
    // Must start with "XN--" and have at least one character after the prefix
    if (name.length() < 5 || name.substr(0, 4) != "XN--") {
        return false;
    }
    
    // Check the part after "XN--" follows valid format
    std::string afterPrefix = name.substr(4);
    static const std::regex PUNYCODE_SUFFIX("^[A-Z0-9]+(-[A-Z0-9]+)*$");
    return std::regex_match(afterPrefix, PUNYCODE_SUFFIX);
}

bool IsSubNameValid(const std::string& name)
{
    // DNS RFC compliant validation - position restrictions are built into the regex
    bool basicValid = std::regex_match(name, SUB_NAME_CHARACTERS);
    
    if (!basicValid) {
        return false;
    }
    
    // Special handling for Punycode: XN-- is allowed to have double hyphens
    if (IsValidUppercasePunycode(name)) {
        return true;
    }
    
    // For non-Punycode names, apply standard hyphen restrictions
    return !std::regex_match(name, DOUBLE_HYPHEN)
           && !std::regex_match(name, LEADING_HYPHEN)
           && !std::regex_match(name, TRAILING_HYPHEN);
}
...
bool IsMsgChannelTagValid(const std::string &tag)
{
    return std::regex_match(tag, MSG_CHANNEL_TAG_CHARACTERS)
        && !std::regex_match(tag, DOUBLE_HYPHEN)
        && !std::regex_match(tag, LEADING_HYPHEN)
        && !std::regex_match(tag, TRAILING_HYPHEN);
}
...
bool IsTypeCheckNameValid(const AssetType type, const std::string& name, std::string& error)
{
    if (type == AssetType::UNIQUE) {
        ...
        if (!valid) { error = "Unique name contains invalid characters (Valid characters are: A-Z a-z 0-9 @ $ % & * ( ) [ ] { } ? : -)";  return false; }
        return true;
        ...
        if (!valid) { error = "Message Channel name contains invalid characters (Valid characters are: A-Z 0-9 -) (must start with letter, end with letter/digit, hyphen not at start/end)";  return false; }
        return true;
    } else if (type == AssetType::OWNER) {
        if (name.size() > MAX_NAME_LENGTH) { error = "Name is greater than max length of " + std::to_string(MAX_NAME_LENGTH); return false; }
        bool valid = IsNameValidBeforeTag(name.substr(0, name.size() - 1));
        if (!valid) { error = "Owner name contains invalid characters (Valid characters are: A-Z 0-9 -) (must start with letter, end with letter/digit, hyphen not at start/end)";  return false; }
        return true;
        ...
        if (!valid) { error = "Vote name contains invalid characters (Valid characters are: A-Z 0-9 -) (must start with letter, end with letter/digit, hyphen not at start/end)";  return false; }
        return true;
    } else if (type == AssetType::QUALIFIER || type == AssetType::SUB_QUALIFIER) {
        if (name.size() > MAX_NAME_LENGTH) { error = "Name is greater than max length of " + std::to_string(MAX_NAME_LENGTH); return false; }
        bool valid = IsQualifierNameValidBeforeTag(name);
        if (!valid) { error = "Qualifier name contains invalid characters (Valid characters are: A-Z 0-9 -) (# must be the first character, must start with letter after #, end with letter/digit, hyphen not at start/end)";  return false; }
        return true;
    } else if (type == AssetType::RESTRICTED) {
        if (name.size() > MAX_NAME_LENGTH) { error = "Name is greater than max length of " + std::to_string(MAX_NAME_LENGTH); return false; }
        bool valid = IsRestrictedNameValid(name);
        if (!valid) { error = "Restricted name contains invalid characters (Valid characters are: A-Z 0-9 -) ($ must be the first character, must start with letter after $, end with letter/digit, hyphen not at start/end)";  return false; }
        return true;
    } else {
...
void CNewAsset::ConstructTransaction(CScript& script) const
{
    ...
    vchMessage.push_back(PHI_R); // r
    vchMessage.push_back(PHI_V); // v
    vchMessage.push_back(PHI_N); // n
    vchMessage.push_back(PHI_Q); // q
...
bool TransferAssetFromScript(const CScript& scriptPubKey, CAssetTransfer& assetTransfer, std::string& strAddress)
{
    ...
    if (AreTransferScriptsSizeDeployed()) {
        // Before phihash activation we used the hardcoded 31 to find the data
        // This created a bug where large transfers scripts would fail to serialize.
        // This fixes that issue (https://github.com/PhicoinProject/phicoin/issues/752)
        // TODO, after the phihash fork goes active, we should be able to remove this if/else statement and just use this line.
...
{
    // Must contain at least 3 outpoints (PHI burn, owner change and one or more new unique assets that share a root (should be in trailing position))
    if (vout.size() < 3) {
...
bool CTransaction::VerifyNewAsset(std::string& strError) const {
    // Issuing an Asset must contain at least 3 CTxOut( PHICOIN Burn Tx, Any Number of other Outputs ..., Owner Asset Tx, New Asset Tx)
    if (vout.size() < 3) {
...
{
    // Issuing an Asset must contain at least 2 CTxOut( PHICOIN Burn Tx, New Asset Tx, Any Number of other Outputs...)
    if (vout.size() < 2) {
...
bool CTransaction::VerifyNewRestrictedAsset(std::string& strError) const {
    // Issuing a restricted asset must cointain at least 4 CTxOut(PHICOIN Burn Tx, Asset Creation, Root Owner Token Transfer, and CNullAssetTxVerifierString)
    if (vout.size() < 4) {
...
{
    // Reissuing an Asset must contain at least 3 CTxOut ( PHICOIN Burn Tx, Any Number of other Outputs ..., Reissue Asset Tx, Owner Asset Change Tx)
    if (vout.size() < 3) {
...
{
    // Check the first transaction and verify that the correct PHI Amount
...
{
    // Verify 'phiq' is in the transaction
...
{
    // Verify 'phir' is in the transaction
    CScript scriptPubKey = txOut.scriptPubKey;
...
bool CreateAssetTransaction(CWallet* pwallet, CCoinControl& coinControl, const std::vector<CNewAsset> assets, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::string* verifier_string)
{
    ...
    if (!change_address.empty()) {
        ...
        if (!IsValidDestination(destination)) {
            error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid PHICOIN address: ") + change_address);
            return false;
        }
    } else {
    ...
    // Check to make sure the wallet has the PHI required by the burnAmount
...
bool CreateReissueAssetTransaction(CWallet* pwallet, CCoinControl& coinControl, const CReissueAsset& reissueAsset, const std::string& address, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::string* verifier_string)
{
    ...
    if (!IsValidDestinationString(address)) {
        error = std::make_pair(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid PHICOIN address: ") + address);
        return false;
    }
...
bool CreateTransferAssetTransaction(CWallet* pwallet, const CCoinControl& coinControl, const std::vector< std::pair<CAssetTransfer, std::string> >vTransfers, const std::string& changeAddress, std::pair<int, std::string>& error, CWalletTx& wtxNew, CReserveKey& reservekey, CAmount& nFeeRequired, std::vector<std::pair<CNullAssetTxData, std::string> >* nullAssetTxData, std::vector<CNullAssetTxData>* nullGlobalRestrictionData)
{
    ...
    CAmount curBalance = pwallet->GetBalance();
    if (curBalance == 0) {
        error = std::make_pair(RPC_WALLET_INSUFFICIENT_FUNDS, std::string("This wallet doesn't contain any PHI, transfering an asset requires a network fee"));
        return false;
    }
    ...
    // If assetTxData is not nullptr, the user wants to add some OP_PHI_ASSET data transactions into the transaction
    if (nullAssetTxData) {
        std::string strError = "";
        int nAddTagCount = 0;
    ...
    // nullGlobalRestiotionData, the user wants to add OP_PHI_ASSET OP_PHI_ASSET OP_PHI_ASSETS data transaction to the transaction
...
bool ContextualCheckReissueAsset(CAssetsCache* assetCache, const CReissueAsset& reissue_asset, std::string& strError, const CTransaction& tx)
{
    ...
    // Check previous asset data with the reissued data
    ...
    // Additional checks for restricted assets
    if (IsAssetNameAnRestricted(reissue_asset.strName)) {
        ...
        if (!tx.GetVerifierStringFromTx(new_verifier, strError, fNotFound)) {
            // If it returns false for any reason other than not being found, fail the transaction check
            if (!fNotFound) {
                return false;
            }
        }
        ...
        if (reissue_asset.nAmount > 0) {
            ...
            if (fNotFound) {
                ...
                if (assetCache->GetAssetVerifierStringIfExists(reissue_asset.strName, current_verifier)) {
                    ...
                    error("%s : failed to get verifier string from a restricted asset, database is out of sync. Reindex required. Please report this to the development team. Asset name: %s, txhash: %s", __func__, reissue_asset.strName, tx.GetHash().GetHex());
                    strError = "failed to get verifier string from a restricted asset, database is out of sync. Reindex required. Please report this to the development team";
                    return false;
                }
            } else {
