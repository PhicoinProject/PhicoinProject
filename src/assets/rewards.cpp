...
bool GenerateDistributionList(const CRewardSnapshot& p_rewardSnapshot, std::vector<OwnerAndAmount>& vecDistributionList)
{
    ...
    CAmount srcUnitDivisor = COIN;  //  Default to divisor for PHI
    ...
    if (p_rewardSnapshot.strDistributionAsset != "PHI") {
        ...
        LogPrint(BCLog::REWARDS, "%s: Distribution is PHI with divisor %d\n", __func__, srcUnitDivisor);
    }
        ...
        std::string& change_address, uint256& retTxid)
{
    ...
    //  Handle payouts using PHI differently from those using an asset
    if (p_rewardSnapshot.strDistributionAsset == "PHI") {
        // Check amount
        ...
        for (int i = start; i < (int)p_pendingPayments.size() && i < stop; i++) {
            ...
            // Parse PHICOIN address (already validated during ownership snapshot creation)
