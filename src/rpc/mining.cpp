...
std::map<std::string, CBlock> mapPHIKAWBlockTemplates;
...
UniValue generateBlocks(std::shared_ptr<CReserveScript> coinbaseScript, int nGenerate, uint64_t nMaxTries, bool keepScript)
{
    ...
    while (nHeight < nHeightEnd)
    {
        ...
        // PHIHASH Assign the mix_hash to the block that was found
...
UniValue generatetoaddress(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() < 2 || request.params.size() > 3)
        throw std::runtime_error(
            ...
            "2. address      (string, required) The address to send the newly generated phicoin to.\n"
...
UniValue getmininginfo(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "  \"errors\": \"...\"            (string) DEPRECATED. Same as warnings. Only shown when phicoind is started with -deprecatedrpc=getmininginfo\n"
            "}\n"
            "\nExamples:\n"
...
// NOTE: Unlike wallet RPC (which use PHI values), mining RPCs follow GBT (BIP 22) in using satoshi amounts
...
UniValue getblocktemplate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() > 1)
        throw std::runtime_error(
            ...
            "  \"DevAutonomousAddress\" : n, (string) Community Autonomous Address\n"
            "  \"DevAutonomousValue\" : n,   (numeric) Community Autonomous Value, 10% of the coinbase\n"
    ...
    if (g_connman->GetNodeCount(CConnman::CONNECTIONS_ALL) == 0 && !gArgs.GetBoolArg("-bypassdownload", false))
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "PHICOIN is not connected!");

    if (IsInitialBlockDownload() && !gArgs.GetBoolArg("-bypassdownload", false))
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "PHICOIN is downloading blocks...");
...
//    const struct VBDeploymentInfo& segwit_info = VersionBitsDeploymentInfo[Consensus::DEPLOYMENT_SEGWIT];
    ...
    result.push_back(Pair("CommunityAutonomousAddress", GetParams().DevAddress()));
    ...
    if (pblock->nTime >= nPHIHASHActivationTime) {
        ...
        if (IsValidDestinationString(address)) {
            ...
            if (mapPHIKAWBlockTemplates.count(lastheader)) {
                if (pblock->nTime - 30 < mapPHIKAWBlockTemplates.at(lastheader).nTime) {
            ...
            result.pushKV("pprpcheader", pblock->GetPHIHASHHeaderHash().GetHex());
            result.pushKV("pprpcepoch", ethash::get_epoch_number(pblock->nHeight));
            mapPHIKAWBlockTemplates[pblock->GetPHIHASHHeaderHash().GetHex()] = *pblock;
            lastheader = pblock->GetPHIHASHHeaderHash().GetHex();
        }
    }
...
static UniValue getphihashhash(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() < 4) {
        throw std::runtime_error(
                "getphihashhash \"header_hash\" \"mix_hash\" nonce, height, \"target\"\n"
                "\nGet the phihash hash for a block given its block data\n"
                ...
                "\nResult:\n"
                "\nExamples:\n"
                + HelpExampleCli("getphihashhash", "\"header_hash\" \"mix_hash\" \"0x100000\" 2456")
                + HelpExampleRpc("getphihashhash", "\"header_hash\" \"mix_hash\" \"0x100000\" 2456")
        );
    }
    ...
    // PhiHash hash
    const auto result = phihash::hash(*context, nHeight, header_hash, nNonce);
...
static UniValue pprpcsb(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() != 3) {
        throw std::runtime_error(
                ...
                "\nAttempts to submit new block to network mined by phihash gpu miner via rpc.\n"
    ...
    if (!mapPHIKAWBlockTemplates.count(header_hash))
        throw JSONRPCError(RPC_INVALID_PARAMS, "Block header hash not found in block data");

    std::shared_ptr<CBlock> blockptr = std::make_shared<CBlock>();
    *blockptr = mapPHIKAWBlockTemplates.at(header_hash);
...
UniValue estimatefee(const JSONRPCRequest& request)
{
    ...
    if (!IsDeprecatedRPCEnabled("estimatefee")) {
        throw JSONRPCError(RPC_METHOD_DEPRECATED, "estimatefee is deprecated and will be fully removed in v0.17. "
            "To use estimatefee in v0.16, restart phicoind with -deprecatedrpc=estimatefee.\n"
...
UniValue getgenerate(const JSONRPCRequest& request)
{
    if (request.fHelp || request.params.size() != 0)
        throw std::runtime_error(
            ...
            "It is set with the command line argument -gen (or " + std::string(PHICOIN_CONF_FILENAME) + " setting gen)\n"
...
UniValue setgenerate(const JSONRPCRequest& request)
{
    ...
    int numCores = GeneratePHICOINs(fGenerate, nGenProcLimit, GetParams());
...
{ //  category              name                      actor (function)         argNames
  //  --------------------- ------------------------  -----------------------  ----------
    ...
    { "mining",             "getphihashhash",          &getphihashhash,          {"header_hash", "mix_hash", "nonce", "height"} },
