...
The PHICOIN Core daemon can be configured to act as a trusted "border
router", implementing the phicoin wire protocol and relay, making
...
The ZeroMQ feature in PHICOIN Core requires the ZeroMQ API >= 4.0.0
...
during the *configure* step of building phicoind:
...
For instance:
    ...
    $ phicoind -zmqpubhashtx=tcp://127.0.0.1:28766 \
               -zmqpubrawtx=ipc:///tmp/phicoind.tx.raw
...
These options can also be provided in phicoin.conf.
...
From the perspective of phicoind, the ZeroMQ socket is write-only; PUB
sockets don't even have a read function. Thus, there is no state
introduced into phicoind directly. Furthermore, no information is
...
using. PHICOINd appends an up-counting sequence number to each
