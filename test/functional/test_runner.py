...
# Copyright (c) 2017-2021 The PHICOIN Core developers
...
`test/functional/test_framework/test_framework.py:PHICOINTestFramework.main`.
...
def main():
    ...
    tmpdir = "%s/phicoin_test_runner_%s" % (args.tmpdirprefix, datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))
    os.makedirs(tmpdir)
    ...
    # Check that the build was configured with wallet, utils, and phicoind
    enable_wallet = config["components"].getboolean("ENABLE_WALLET")
    enable_cli = config["components"].getboolean("ENABLE_UTILS")
    enable_phicoind = config["components"].getboolean("ENABLE_PHICOIND")
    if not (enable_wallet and enable_cli and enable_phicoind):
        print("No functional tests to run. Wallet, utils, and phicoind must all be enabled")
...
def run_tests(test_list, src_dir, build_dir, exeext, tmpdir, use_term_control, jobs=1, enable_coverage=False, args=None, combined_logs_len=0, failfast=False, last_loop=False):
    # Warn if phicoind is already running (unix only)
    if args is None:
        args = []
    try:
        if subprocess.check_output(["pidof", "phicoind"]) is not None:
            print("%sWARNING!%s There is already a phicoind process running on this system. Tests may fail unexpectedly due to resource contention!" % (BOLD[1], BOLD[0]))
    ...
    #Set env vars
    if "PHICOIND" not in os.environ:
        os.environ["PHICOIND"] = build_dir + '/src/phicoind' + exeext
        os.environ["PHICOINCLI"] = build_dir + '/src/phicoin-cli' + exeext
...
class RPCCoverage:
    ...
    commands per `phicoin-cli help` (`rpc_interface.txt`).
