...
The RPM spec file provided here is for PHICOIN-Core 0.12.0 and builds on CentOS
...
reference the release tarball available from https://phicoin.net/bin/ and
`Source1` should reference the BerkeleyDB source.

Sources 10-99 are for source files that are maintained in the
[PHICOIN git repository](https://github.com/PhicoinProject/phicoin) but are not part of
...
Currently only `Source10` is used, for the example `phicoin.conf` file.
...
in the phicoin git repository. At present only an SVG version of the PHICOIN
icon is used.

## Patches

In general, patches should be avoided. When a packager feels a patch is
necessary, the packager should bring the problem to the attention of the phicoin
...
needed when 0.12.1 is released, a proper fix is already in the PHICOIN git
master branch.
...
idea to build PHICOIN Core against a static version of that library compiled
...
The `phicoin.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `phicoin-server` package. At this point, most RPM based
...
The `phicoin-server` package should have SELinux support. How to properly do
...
PHICOIN-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/phicoin/). He has been
packaging PHICOIN for Fedora at least since 2012.
...
4. I renamed the `phicoin` package that contains the Qt GUI to `phicoin-core` as
that appears to be how the general population refers to it, in contrast to
`phicoin-xt` or `phicoin-classic`. I wanted to make sure the general population
