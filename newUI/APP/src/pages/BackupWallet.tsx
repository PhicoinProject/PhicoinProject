 ...
 * Exports: salt, encrypted sentinel, mnemonic hash (never the plaintext mnemonic).
 */
export const BackupWallet: React.FC = () => {
  ...
  const exportToFile = () => {
    ...
    a.href = url;
    a.download = `phicoin-backup-${new Date().toISOString().split('T')[0]}.json`;
    a.click();
    ...
    <div className="mx-auto max-w-2xl p-6">
      ...
      <div className="mb-6 rounded-lg border border-amber-200 dark:border-amber-600 bg-amber-50 dark:bg-amber-900/20 p-4">
        ...
        <ul className="mt-2 list-inside list-disc text-sm text-amber-700 dark:text-amber-400 space-y-1">
          <li>Never store this file online or share it</li>
          <li>Keep multiple copies in different physical locations</li>
          <li>You will need your passphrase to restore from this backup</li>
          <li>This backup does not contain your passphrase</li>
        </ul>
      </div>
      ...
      <p className="mb-2 text-sm text-gray-600 dark:text-dark-mutedText">
        Backup contains encrypted wallet data. Use the original passphrase to restore.
      </p>
