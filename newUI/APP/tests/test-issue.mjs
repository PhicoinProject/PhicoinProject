/**
 * Broadcast asset issuance with known UTXO (hardcoded to avoid getaddressutxos issues)
 */
...
function buildRawTxHex(inputs, outputs, version = 2, locktime = 0) {
  ...
  for (const inp of inputs) {
    parts.push(hexToArray(inp.txid));
...
function buildAssetScriptWithP2PKH(h160, magic, payload) {
  ...
  script.set(p2pkh, 0);
  script[25] = 0xc0;
  script[26] = fullPayload.length;
  script.set(fullPayload, 27);
  script[27 + fullPayload.length] = 0x61;
  return script;
}
...
function serializeCNewAsset(name, amount, units, reissuable, hasIPFS, ipfsHash = '') {
  ...
  let off = 0;
  data[off++] = nameBytes.length;
  data.set(nameBytes, off); off += nameBytes.length;
  const amtBuf = new Uint8Array(8);
  new DataView(amtBuf.buffer).setBigInt64(0, BigInt(amount), true);
  data.set(amtBuf, off); off += 8;
  data[off++] = units; data[off++] = reissuable; data[off++] = hasIPFS;
  if (ipfsHash) {
    const ipfsBytes = new TextEncoder().encode(ipfsHash);
    data[off++] = ipfsBytes.length;
    data.set(ipfsBytes, off); off += ipfsBytes.length;
  }
  return data;
}
...
function serializeCAssetTransfer(name, amount) {
  const nameBytes = new TextEncoder().encode(name);
  const data = new Uint8Array(1 + nameBytes.length + 8);
  let off = 0;
  data[off++] = nameBytes.length;
  data.set(nameBytes, off); off += nameBytes.length;
  const amtBuf = new Uint8Array(8);
...
addrPayload[0] = PUB_KEY_HASH; addrPayload.set(h160, 1);
...
// Known UTXO from gettxout
const utxoTxid = '400b92a42fd7c746f49e78a2cc58abb549d4e90eb4a10af4d6defc32227cf740';
const utxoVout = 0;
const utxoSatoshis = 1 * 1e8; // 1 PHI
const utxoScript = '76a914ef7bce8bd1c9244ff7d1e7bb7199ec1fe084bf3f88ac';

// Burn address script
const burnScript = hexToArray('76a9148684a6449c157dd0a2f393fc5147e47cd4fd9f2588ac');

console.log('=== Asset Issue Broadcast ===');
console.log('Address:', address);
console.log('UTXO:', utxoTxid, 'vout:', utxoVout, 'sat:', utxoSatoshis);
...
const signedHex = signRawTransaction(rawHex, [{
  txid: utxoTxid,
  vout: utxoVout,
  scriptPubKey: hexToArray(utxoScript),
  privateKey: pk,
}]);
if (!signedHex) throw new Error('Signing returned null');

// testmempoolaccept (skip on missing-inputs since daemon may not track the address)
try {
  const mempoolResult = await rpcCall('testmempoolaccept', [[signedHex], false]);
  console.log('testmempoolaccept:', JSON.stringify(mempoolResult[0]));
  if (mempoolResult[0].allowed !== true) {
    const reason = mempoolResult[0]['reject-reason'] ?? 'unknown';
    if (reason !== 'missing-inputs') {
      console.log('Rejected:', reason);
      process.exit(1);
    }
    console.log('Skipping testmempoolaccept (missing-inputs - daemon address tracking issue), proceeding to broadcast...');
  }
} catch (e) {
